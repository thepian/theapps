...
from theapps.tagging.models import Tag, TaggedItem
from theapps.tagging.utils import LINEAR, LOGARITHMIC
...
class TagsForModelNode(Node):
    ...
    def render(self, context):
        model = models.get_model(*self.model.split('.'))
        if model is None:
        ...
        try:
            ...
            raise TemplateSyntaxError(_('Cannot render tags for model without tags field: %s') % self.model)
        context[self.context_var] = tags_field.Tag.objects.usage_for_model(model, counts=self.counts)
        return ''
...
class TagCloudForModelNode(Node):
    ...
    def render(self, context):
        ...
        context[self.context_var] = \
            tags_field.Tag.objects.cloud_for_model(model, **self.kwargs)
        return ''
...
class TagsForObjectNode(Node):
    ...
    def render(self, context):
        ...
        try:
            tags_field = obj._meta.get_field('tags')
        except AttributeError:
            raise TemplateSyntaxError(_('Cannot render tags for object without tags field: %s') % obj._default_manager.model)
        context[self.context_var] = tags_field.Tag.objects.filter(object=obj)
        return ''
...
class TaggedObjectsNode(Node):
    ...
    def render(self, context):
        ...
        try:
            tags_field = model._meta.tags
        except AttributeError:
            raise TemplateSyntaxError(_('Cannot render tags for model without tags field: %s') % self.model)
        context[self.context_var] = tags_field.Tag.objects.filter(name=self.tag.resolve(context))
        return ''
